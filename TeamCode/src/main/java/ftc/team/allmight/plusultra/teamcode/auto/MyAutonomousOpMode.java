...
public class MyAutonomousOpMode extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        // Initialize DriveConstants (adjust params)
        ...
        rrHelper = new RoadRunnerHelper(
                hardwareMap, "left", "right", driveParams,
        ...
        if (waypoints.size() > 1) {
            ...
            for (int i = 1; i < waypoints.size() && opModeIsActive(); i++) {
                ...
                // Run action manually
                while (opModeIsActive() && segAction.run(new com.acmerobotics.dashboard.telemetry.TelemetryPacket())) {
                    rrHelper.update(dt);
                    drive.updatePoseEstimate();
