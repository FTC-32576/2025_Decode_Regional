...
//import ftc.team.allmight.plusultra.teamcode.subsystems.DriveTest;
...
//
//@Autonomous(name = "AUTO - TESTE Simples (Refactor Drive)", group = "A")
...
//
//        // DRIVE (refatorado)
//        DriveTest drive = new DriveTest(hardwareMap, imu);
...
//
//        // novo: move com PID síncrono (bloqueante) - timeout em ms (ajuste conforme necessário)
//        drive.moveToDistancePID(-135.0, 1.0, 8000, telemetry);
//
//        // shoot inicial
//        shootar(5, 0.45678901, 0, shooter, servo, Alliance.RED, shootNumber.START); // shootar 3 bolas + 1 por segurança
//
//        // giro refinado (turnIMU com timeout)
//        drive.turnIMU(50.0, 0.35, 1.0, telemetry, 2500);
//
//        // voltar pra linha de bolas (síncrono)
//        drive.moveToDistancePID(-46.0, 0.95, 4000, telemetry);
//        sleep(35);
//
//        // girar pra linha de bolas PPG
//        drive.turnIMU(-90.0, 0.39, 6.0, telemetry, 3000);
//        sleep(35); // espera antes de andar pra ser preciso
//
//        // andar pra frente até linha (síncrono)
//        drive.moveToDistancePID(48.5, 0.94, 4000, telemetry);
//
//        // INTAKE durante movimento assíncrono (híbrido)
//        intake.intake(); // liga intake
//        intake.update();
//
//        // iniciar movimento assíncrono para coletar
//        drive.moveToDistanceAsync(53.0, 0.22, 5000); // timeout 5s (ajuste conforme comportamento)
//
//        // loop de coleta assíncrono: mantém drive e intake atualizados
//        while (opModeIsActive() && (intake.intakeIsBusy() || drive.moveIsBusy())) {
//            drive.moveToDistanceAsyncUpdate();   // mantém o drive andando
...
//        intake.update();
//
//        // voltar pra trás (síncrono)
//        drive.moveToDistancePID(-100.0, 1.0, 7000, telemetry);
//
//        // pequeno giro de aproximação
//        drive.turnIMU(45.0, 0.7, 1.0, telemetry, 2500);
//
...
//                } else{
//                    // aguarda shooter estabilizar (com timeout de segurança)
//                    long timeout = System.currentTimeMillis() + 3250; // 3.25s máximo para estabilizar
//                    while (opModeIsActive() && System.currentTimeMillis() < timeout) {
//                        double erro = adjustShooterPower(shooter, alliance);
//                        if (erro < STABLE_THRESHOLD) break;
//                    }
...
//            } else{
//                // aguarda shooter estabilizar (com timeout de segurança)
//                long timeout = System.currentTimeMillis() + 1600; // 1.6s máximo para estabilizar
//                while (opModeIsActive() && System.currentTimeMillis() < timeout) {
//                    double erro = adjustShooterPower(shooter, alliance);
//                    if (erro < STABLE_THRESHOLD) break;
//                }
